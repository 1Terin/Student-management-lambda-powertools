...
```bash
lambdapowertools-sam$ sam build
```
...
```bash
lambdapowertools-sam$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
lambdapowertools-sam$ sam local start-api
lambdapowertools-sam$ curl http://localhost:3000/
```
...
```bash
lambdapowertools-sam$ sam logs -n HelloWorldFunction --stack-name lambdapowertools-sam --tail
```
...
```bash
lambdapowertools-sam$ cd hello-world
...
```bash
sam delete --stack-name lambdapowertools-sam
```
